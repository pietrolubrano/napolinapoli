...
export default function BookingHeader({
    ...
    return (
        <div className="grid md:grid-cols-4 mb-8 gap-1 md:gap-6 px-4 max-w-lg mx-auto text-gray-600">
            ...
            <div className="flex md:flex-col md:justify-center items-center text-center gap-3 font-bold">
                <FaPerson className="text-background text-xl md:text-4xl" />{lang === "it" ? "N. Persone:" : "Guests:"} <br className="hidden md:block" /> {guests}
            </div>
            <div className="flex md:flex-col md:justify-center items-center text-center gap-3 font-bold">
                <FaDoorOpen className="text-background text-xl md:text-4xl" />{lang === "it" ? "Stanza:" : "Room:"} <br className="hidden md:block" />{apartment.name}
            </div>
        </div>
    )
}
